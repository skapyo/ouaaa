  ...
  return (
    <AppLayout>
      <Head>
        ...
        <meta name="description" content={data && (`${data.event.label} ${data.event.city} ${data}` && `${momentTimezone(parseInt(data.event.startedAt)).format('DD/MM/YYYY HH:mm:ssZZ')} ${momentTimezone(parseInt(data.event.endedAt)).tz('France/Paris').format('DD/MM/YYYY HH:mm')}${showCategory(data.event.entries)}`)} />
        {data
