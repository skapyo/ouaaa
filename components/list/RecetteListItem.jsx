...
export default function RecetteListItem(props) {
  const { recette } = props;

  const image = recette?.pictures?.filter((a, b) => (a.logo ? -1 : 1))?.[0]?.originalPicturePath || '/icons/planet.svg';
  moment.locale('fr');
  const date = moment(parseInt(recette.createdAt)).format('dddd DD MMMM YYYY');

  const myLoader = ({ src, width, quality }) => {
    return src.startsWith('/static') ? `${process.env.NEXT_PUBLIC_URI}${src}?w=${width}&q=${quality || 75}` : src;
  };

  const link = `/recette/${recette.id}`;
